...
import { collections, type Collection } from '@/lib/cms/config';
...
type PageListProps = {
  ...
  renderCreateForm: (collection: Collection, entries: CmsFile[]) => ReactNode;
};
        ...
        {collections.map((collection) => {
          ...
          return (
            <section key={collection.id} className="mb-3">
              ...
              {creatingIn === collection.id ? renderCreateForm(collection, entries) : null}
              <ul>
                {visible.map((file) => (
                  <li key={file.path}>
                    <button
